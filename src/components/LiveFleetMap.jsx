...
import { Filter, LocateFixed, Maximize2, Minus, Plus, Radar, RotateCcw, Truck, X } from 'lucide-react';
...
const COAST_PATH = 'M 0 0 L 0 100 L 20 100 C 24 92, 28 86, 32 79 C 36 72, 40 68, 46 62 C 51 56, 55 50, 60 43 C 66 35, 73 26, 81 17 C 88 10, 94 5, 100 3 L 100 0 Z';
...
const getRouteDots = (route) =>
  route.points.slice(0, -1).map((point, index) => {
    const nextPoint = route.points[index + 1];
    return {
      key: `${route.id}-${index}`,
      mapX: (point.mapX + nextPoint.mapX) / 2,
      mapY: (point.mapY + nextPoint.mapY) / 2,
    };
  });
...
function MapCanvas({
  ...
  return (
    <div className={`live-fleet-map-stage ${fullscreen ? 'fullscreen' : ''}`}>
      <div
        className={`live-fleet-map-canvas ${fullscreen ? 'fullscreen' : ''}`}
        ...
        <svg viewBox="0 0 100 100" className={`live-fleet-map-svg ${showTraffic ? 'traffic-on' : ''}`} aria-label="Live fleet map">
          <defs>
            <linearGradient id="oceanGradient" x1="0%" x2="100%" y1="0%" y2="100%">
              ...
              <stop offset="100%" stopColor="#9dcaf7" />
            </linearGradient>
          ...
          <path d={COAST_PATH} fill="url(#oceanGradient)" opacity="0.92" />
          ...
          {TRACKING_ROUTE_CORRIDORS.map((route) => (
            <g key={route.id}>
              ...
              {getRouteDots(route).map((dot) => (
                <circle key={dot.key} cx={dot.mapX} cy={dot.mapY} r="0.38" className="fleet-route-direction-dot" />
              ))}
            </g>
          ))}
          ...
          {selectedVehicle && (
            <g>
              ...
              {getRouteDots({ id: 'selected', points: selectedVehicle.routePoints }).map((dot) => (
                <circle key={dot.key} cx={dot.mapX} cy={dot.mapY} r="0.46" className="fleet-route-direction-dot active" />
              ))}
            </g>
          )}
        ...
        {vehicles.map((vehicle, index) => {
          ...
          return (
            <button
              ...
              type="button"
              className={`fleet-marker ${isSelected ? 'selected' : ''} ${compact ? 'compact' : ''} ${vehicle.status === 'On Route' ? 'live' : ''}`}
              style={{
                left: `${vehicle.mapX}%`,
                top: `${vehicle.mapY}%`,
...
export default function LiveFleetMap({
  ...
  return (
    <>
      <div className={`live-fleet-map-card ${compact ? 'compact' : 'full'}`}>
